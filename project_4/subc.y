...
compound_stmt:  '{'	
				...
				local_defs
		{
			...
			sprintf(code_gen, "\tshift_sp %d", scope_top->scope_entry->offset);
			yyprint(code_gen);
			sprintf(code_gen, "%s_block%d_start:", func_name, block_num);
			yyprint(code_gen);
		}
				stmt_list	
		{
...
expr:
                ...
                $$ = assigndecl($1, $4);
		if($$!=NULL){
			sprintf(code_gen, "\tassign");
			yyprint(code_gen);
			sprintf(code_gen, "\tfetch");
			yyprint(code_gen);
		}
        }
                | or_expr{
...
unary:          '(' expr ')'{
		...
		sprintf(code_gen, "\tpush_const %d", $1);
		yyprint(code_gen);
		...
		sprintf(code_gen, "\tpush_const %s", $1);
		yyprint(code_gen);
        }
                | ID{
                REDUCE("unary->ID");
                $$ = findcurrentdecl($1);
		if($$ !=NULL){
			sprintf(code_gen, "\tpush_reg fp");
			yyprint(code_gen);
			sprintf(code_gen, "\tpush_const %d",$$->offset+1);
			yyprint(code_gen);
			sprintf(code_gen, "\tadd");
			yyprint(code_gen);
		}
        }
                | STRING{
		...
		sprintf(code_gen, "Str%d. string %s", string_num, $1);
		yyprint(code_gen);
		sprintf(code_gen, "\tpush_const Str%d",string_num++);
		yyprint(code_gen);
        }
                | '-' unary %prec '!' {
                REDUCE("unary->'-' unary");
